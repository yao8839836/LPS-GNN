...
for i in range(100):#sample_sub_graph_qq
    t = time.time()
    adj, adj_aug, features, node_ids = process.load_subgraph_aug_edge("dapan/qq/subgraph_edges/" + str(i), "dapan/qq/subgraph_edges_aug/" + str(i),
    "dapan/qq/subgraph_features/" + str(i), ft_size)
    qq_subgraph_list.append((adj, adj_aug, features, node_ids))  
    print("load qq subgraph time:", time.time() - t)

wechat_subgraph_list = []
for i in range(200):#sample_sub_graph_wechat
    t = time.time()
    adj, adj_aug, features, node_ids = process.load_subgraph_aug_edge("dapan/wechat/subgraph_edges/" + str(i), "dapan/wechat/subgraph_edges_aug/" + str(i), 
    "dapan/wechat/subgraph_features/" + str(i), ft_size)
    wechat_subgraph_list.append((adj, adj_aug, features, node_ids))    
...
for epoch in range(nb_epochs):
    ...
    avg_loss = 0
    for (adj, adj_aug, features, node_ids) in qq_subgraph_list:
        ...
        adj_aug = process.normalize_adj(adj_aug + sp.eye(adj_aug.shape[0]))

        if sparse:
            sp_adj = process.sparse_mx_to_torch_sparse_tensor(adj)
            sp_adj_aug = process.sparse_mx_to_torch_sparse_tensor(adj_aug)            
        else:
            adj = (adj + sp.eye(adj.shape[0])).todense()
            adj_aug = (adj_aug + sp.eye(adj_aug.shape[0])).todense()

        features = torch.FloatTensor(features[np.newaxis])
        if not sparse:
            adj = torch.FloatTensor(adj[np.newaxis])
            adj_aug = torch.FloatTensor(adj_aug[np.newaxis])
        ...
        if torch.cuda.is_available():
            ...
            if sparse:
                sp_adj = sp_adj.cuda()
                sp_adj_aug = sp_adj_aug.cuda()                
            else:
                adj = adj.cuda()
                adj_aug = adj_aug.cuda()
        model.train()
        optimiser.zero_grad()
        ...
        logits = model(features, sp_adj if sparse else adj, sp_adj_aug if sparse else adj_aug,
    ...
    for (adj, adj_aug, features, node_ids) in wechat_subgraph_list:
